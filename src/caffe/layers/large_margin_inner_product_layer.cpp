...
void LargeMarginInnerProductLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  type_ = this->layer_param_.large_margin_inner_product_param().type();
  iter_ = this->layer_param_.large_margin_inner_product_param().iteration();
  lambda_ = (Dtype)0.;
...
void LargeMarginInnerProductLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
  Dtype base_ = this->layer_param_.large_margin_inner_product_param().base();
  Dtype gamma_ = this->layer_param_.large_margin_inner_product_param().gamma();
  Dtype power_ = this->layer_param_.large_margin_inner_product_param().power();
  Dtype lambda_min_ = this->layer_param_.large_margin_inner_product_param().lambda_min();
